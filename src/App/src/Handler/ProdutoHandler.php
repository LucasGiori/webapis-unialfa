...
class ProdutoHandler implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        switch ($method) {
            ...
            case 'POST':
                ...
                if($recordSet->count() <= 0) {
                    return new JsonResponse(["message" => "Não existe nenhuma categoria com o identificador informado"], StatusCodeInterface::STATUS_UNPROCESSABLE_ENTITY);
                }
                ...
                try {
                    ...
                    return new JsonResponse(
                        [
                            "message" => sprintf(
                                "%s - %s","Erro ao executar query. Detalhes: ",
                                $e->getMessage()
                            )
                        ],
            ...
            case 'DELETE':
                if(empty($idproduto) || !is_numeric($idproduto)) {
                    return new JsonResponse(
                        ["message" => "Identificador da produto inválido"],
                        StatusCodeInterface::STATUS_UNPROCESSABLE_ENTITY
                    );
                }
                ...
                if($recordSet->count() <= 0) {
                    return new JsonResponse(["message" => "Não existe nenhum produto com este identificador para excluir"],StatusCodeInterface::STATUS_NOT_FOUND);
                }
        ...
        return new JsonResponse(["message" => "Sem registro de produtos"]);
    }
}